...
export class EmailAgent extends Agent {
    ...
    onProgress;
    onEmailsClassified;
    constructor(agentConfig, emailConfig, notifier) {
        super(agentConfig);
    ...
    initializeLegalAgent() {
        ...
        this.legalAgent = new LegalAgent({
            ...
            enabled: true,
        }, legalConfig, this.notifier);
        console.log('[EmailAgent] Legal Agent integrado');
    }
    initializeFinancialAgent() {
        ...
        try {
            ...
            for (const email of emails) {
                ...
                try {
                    ...
                    console.log(`[EmailAgent] ${this.getPriorityEmoji(classification.priority)} ` +
    ...
    isContractEmail(email) {
        ...
        const hasContractIndicator = contractIndicators.some(indicator => content.includes(indicator));
        if (!hasContractIndicator)
            return false;
    ...
    isFinancialEmail(email) {
        ...
        const hasFinancialIndicator = financialIndicators.some(indicator => content.includes(indicator));
    ...
    hasActionItems(email) {
        ...
        const hasIndicator = actionIndicators.some(indicator => content.includes(indicator));
     ...
     */
    async processWithTaskAgent(email) {
     ...
     */
    async processWithFinancialAgent(email) {
        ...
        try {
            ...
            if (email.attachments && email.attachments.length > 0) {
                ...
                attachmentInfo = email.attachments.map(att => `- ${att.filename} (${att.mimeType}, ${att.size} bytes)`).join('\n');
     ...
     */
    async processWithLegalAgent(email) {
        ...
        try {
            ...
            const attachmentsWithContent = await Promise.all(email.attachments.map(async (att, index) => {
                try {
                    if (!att.id) {
            ...
            const validAttachments = attachmentsWithContent.filter((att) => att !== null && att.content !== undefined);
    ...
    async notifyUrgent(urgentEmails) {
        if (!this.notifier)
            return;
