...
export class CommercialAnalyzer {
  private config: CommercialAgentConfig;
  ...
  isCommercialEmail(subject: string, body: string): boolean {
    ...
    return this.config.commercialKeywords.some(keyword => 
  ...
  async analyze(
    ...
    interface AnalysisResult {
      items: Array<{
        ...
        clientEmail?: string;
        clientPhone?: string;
        clientType: CommercialItem['clientType'];
        title: string;
        description: string;
        ...
        isUrgentBid?: boolean;
        priority: CommercialItem['priority'];
    ...
    const result = await aiClient.analyze<AnalysisResult>(
      emailContext,
      systemPrompt + '\n\nAnalise este email comercial e extraia informações estruturadas.',
      CommercialAnalysisSchema as AITool
    );
