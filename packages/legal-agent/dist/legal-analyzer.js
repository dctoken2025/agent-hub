...
export class LegalAnalyzer {
     ...
     */
    async analyze(document, emailContext, emailId, threadId) {
        const aiClient = getAIClient();
        const documentContext = this.buildDocumentContext(document, emailContext);
        const systemPrompt = this.buildSystemPrompt();
        const result = await aiClient.analyze(documentContext, systemPrompt + '\n\nAnalise o documento e retorne a análise estruturada.', ContractAnalysisSchema);
        if (result) {
            return {
                ...result,
                emailId,
     ...
     */
    buildDocumentContext(document, emailContext) {
        let context = `
...
Lembre-se: Seu objetivo é proteger o cliente e garantir que ele tome uma decisão informada antes de assinar. Sempre deixe claro O QUE precisa ser feito e QUEM deve fazer.`;
     ...
     */
    defaultAnalysis(document, emailId, threadId) {
        return {
            emailId,
            threadId,
...
//# sourceMappingURL=legal-analyzer.js.map