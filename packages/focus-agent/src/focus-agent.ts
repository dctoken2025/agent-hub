import { Agent, type AgentConfig, type AgentResult, getAIClient } from '@agent-hub/core';
import type {
  FocusAgentConfig,
  FocusAgentInput,
  FocusBriefing,
 ...
 */
export class FocusAgent extends Agent<FocusAgentInput, FocusBriefing> {
  private _focusConfig: FocusAgentConfig;
  private dataCollector?: (userId: string, scope: 'today' | 'week') => Promise<CollectedData>;

  constructor(
    agentConfig: AgentConfig,
    focusConfig: FocusAgentConfig = {}
  ) {
    super(agentConfig);
    this._focusConfig = {
   ...
   */
  async execute(input?: FocusAgentInput): Promise<AgentResult<FocusBriefing>> {
    ...
    try {
      ...
      if (totalItems === 0) {
        const emptyBriefing: FocusBriefing = {
          scope,
   ...
   */
  private async analyzeWithAI(data: CollectedData, scope: 'today' | 'week'): Promise<FocusBriefing> {
...
Seja direto, prático e focado em ações concretas.`;
    ...
    try {
      ...
      const prioritizedItems: FocusItem[] = (parsed.items || []).map((item: Record<string, unknown>) => {
        const focusItem: FocusItem = {
          ...
          originalData: this.findOriginalData(item.id as number, item.type as string, originalData),
        };
   ...
   */
  private createFallbackBriefing(data: CollectedData, scope: 'today' | 'week'): FocusBriefing {
   ...
   */
  private getExpirationDate(scope: 'today' | 'week'): Date {
    const now = new Date();
    if (scope === 'today') {
      // Expira à meia-noite
