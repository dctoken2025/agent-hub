...
export interface UsageRecord {
  ...
  inputTokens: number;
  outputTokens: number;
  estimatedCost: number; // em microdólares
  durationMs: number;
  success: boolean;
  errorMessage?: string;
  createdAt: Date;
...
let dbSaveFunction: ((records: UsageRecord[]) => Promise<void>) | null = null;

/**
 * Configura a função de salvamento no banco.
 */
export function setUsageSaveFunction(fn: (records: UsageRecord[]) => Promise<void>): void {
  dbSaveFunction = fn;
}
 ...
 */
export async function forceFlushUsage(): Promise<void> {
  if (flushTimeout) {
