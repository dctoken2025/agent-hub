...
export class FinancialAnalyzer {
     ...
     */
    isFinancialEmail(subject, body) {
        const content = `${subject} ${body}`.toLowerCase();
        return this.config.financialKeywords.some(keyword => content.includes(keyword.toLowerCase()));
    }
    /**
     * Analisa um email financeiro e extrai informações estruturadas.
     * Agora também processa o conteúdo de anexos (PDFs, imagens).
     */
    async analyze(emailSubject, emailBody, emailId, threadId, attachmentInfo, emailFrom, emailDate, extractedDocuments) {
        const aiClient = getAIClient();
        const context = this.buildContext(emailSubject, emailBody, attachmentInfo, extractedDocuments);
        const systemPrompt = this.buildSystemPrompt();
        const result = await aiClient.analyze(context, systemPrompt + '\n\nAnalise este email e seus anexos. Extraia TODAS as informações financeiras disponíveis.', FinancialAnalysisSchema);
     ...
     */
    buildContext(subject, body, attachmentInfo, extractedDocuments) {
        let context = `
═══════════════════════════════════════════════════════════════
...
//# sourceMappingURL=financial-analyzer.js.map