 ...
 */
export class FinancialAgent extends Agent {
    analyzer;
    extractor;
    financialConfig;
    notifier;
    ...
    queue = [];
    constructor(agentConfig, financialConfig, notifier) {
        super(agentConfig);
    ...
    async execute(input) {
        ...
        try {
            ...
            const items = await this.analyzer.analyze(toProcess.emailSubject, toProcess.emailBody, toProcess.emailId, toProcess.threadId, toProcess.attachmentInfo, toProcess.emailFrom, toProcess.emailDate, extractedDocuments.length > 0 ? extractedDocuments : undefined);
     ...
     */
    async notifyUrgent(input, items) {
        if (!this.notifier)
            return;
        const urgentItems = items.filter(i => i.priority === 'urgent' || i.status === 'overdue');
...
//# sourceMappingURL=financial-agent.js.map