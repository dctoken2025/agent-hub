...
export class TaskExtractor {
    ...
    hasActionItems(subject, body) {
        ...
        const hasKeywords = this.config.taskKeywords.some(keyword => content.includes(keyword.toLowerCase()));
     ...
     */
    isVipStakeholder(email) {
        const emailLower = email.toLowerCase();
        return this.config.vipStakeholders.some(vip => emailLower.includes(vip.toLowerCase()));
    }
    /**
     * Extrai action items de um email usando IA.
     */
    async extract(emailSubject, emailBody, emailId, emailFrom, threadId, emailDate) {
        const aiClient = getAIClient();
        const context = this.buildContext(emailSubject, emailBody, emailFrom);
        const systemPrompt = this.buildSystemPrompt();
        const result = await aiClient.analyze(context, systemPrompt, TaskExtractionSchema);
        ...
        return items.map((item) => {
            ...
            const finalPriority = this.calculateFinalPriority(item.priority, stakeholder.importance, deadline);
            const actionItem = {
                emailId,
                threadId,
                emailSubject,
     ...
     */
    async generateReply(emailSubject, _emailBody, emailFrom, items) {
        if (!this.config.generateSuggestedReply || items.length === 0) {
            return undefined;
        }
        const aiClient = getAIClient();
        const itemsList = items.map((item, i) => `${i + 1}. [${item.category.toUpperCase()}] ${item.title}\n   Original: "${item.originalText}"\n   Status: Pendente`).join('\n\n');
...
4. Ter encerramento adequado`;
        try {
            const response = await aiClient.chat([{ role: 'user', content: userMessage }], systemPrompt);
     ...
     */
    buildContext(subject, body, from) {
        return `
=== EMAIL PARA ANÁLISE ===
...
Exemplo ruim: "Item sobre Vortx"`;
     ...
     */
    calculateFinalPriority(aiPriority, stakeholderImportance, deadline) {
        let score = 0;
        ...
        if (stakeholderImportance === 'vip')
            score += 25;
        else if (stakeholderImportance === 'high')
            score += 15;
     ...
     */
    buildPriorityReason(priority, stakeholder, deadline) {
        const reasons = [];
        ...
        if (reasons.length === 0) {
            switch (priority) {
                case 'critical':
                    reasons.push('Item crítico');
                    break;
                case 'high':
                    reasons.push('Alta prioridade');
                    break;
                case 'medium':
                    reasons.push('Prioridade normal');
                    break;
                case 'low':
                    reasons.push('Baixa prioridade');
                    break;
            }
        }
        return reasons.join(' + ');
