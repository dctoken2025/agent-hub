 ...
 */
export class StablecoinAgent extends Agent {
    alchemyClient;
    anomalyDetector;
    stablecoinConfig;
    notifier;
    // Estado interno
    stablecoins = [];
    lastProcessedBlock = new Map(); // address -> block
    lastSupply = new Map(); // address -> supply
    // Callbacks para persistência (serão definidos pela API)
    onEventDetected;
    onAnomalyDetected;
    onSupplySnapshot;
    onAgentLog;
    constructor(agentConfig, stablecoinConfig, notifier) {
        super(agentConfig);
    ...
    removeStablecoin(address) {
        this.stablecoins = this.stablecoins.filter(s => s.address.toLowerCase() !== address.toLowerCase());
    }
    /**
     * Atualiza a API key da Alchemy.
     */
    updateAlchemyApiKey(apiKey) {
     ...
     */
    async processStablecoin(stablecoin, result) {
        ...
        const frequencyAnomaly = this.anomalyDetector.analyzeFrequency(stablecoin.symbol, addressKey, result.events.filter(e => e.stablecoin.address.toLowerCase() === stablecoin.address.toLowerCase()));
        if (frequencyAnomaly) {
     ...
     */
    async processSupply(stablecoin, addressKey, blockNumber, result) {
        try {
            ...
            if (previousSupply) {
                const supplyAnomaly = this.anomalyDetector.analyzeSupplyChange(stablecoin.symbol, currentSupply, previousSupply, stablecoin.decimals);
                if (supplyAnomaly) {
     ...
     */
    convertToStablecoinEvent(raw, stablecoin) {
...
//# sourceMappingURL=stablecoin-agent.js.map