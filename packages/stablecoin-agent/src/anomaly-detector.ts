...
export class AnomalyDetector {
  ...
  private eventHistory: Map<string, StablecoinEvent[]> = new Map(); // stablecoin address -> eventos
   ...
   */
  private checkMintAnomaly(event: StablecoinEvent, amount: number): AnomalyAlert | null {
    // Mint muito grande (> 100M) = CRITICAL
    if (amount >= this.thresholds.largeMint * 10) {
      return this.createAlert(
        'large_mint',
        'critical',
        `Emissão Muito Grande de ${event.stablecoin.symbol}`,
        `Detectado mint de ${event.amountFormatted} ${event.stablecoin.symbol} - 10x acima do threshold normal`,
        event
      );
    }
    ...
    if (amount >= this.thresholds.largeMint) {
      return this.createAlert(
        'large_mint',
        'high',
        `Grande Emissão de ${event.stablecoin.symbol}`,
        `Detectado mint de ${event.amountFormatted} ${event.stablecoin.symbol}`,
        event
      );
    }
   ...
   */
  private checkBurnAnomaly(event: StablecoinEvent, amount: number): AnomalyAlert | null {
    // Burn muito grande (> 100M) = CRITICAL
    if (amount >= this.thresholds.largeBurn * 10) {
      return this.createAlert(
        'large_burn',
        'critical',
        `Queima Muito Grande de ${event.stablecoin.symbol}`,
        `Detectado burn de ${event.amountFormatted} ${event.stablecoin.symbol} - 10x acima do threshold normal`,
        event
      );
    }
    ...
    if (amount >= this.thresholds.largeBurn) {
      return this.createAlert(
        'large_burn',
        'high',
        `Grande Queima de ${event.stablecoin.symbol}`,
        `Detectado burn de ${event.amountFormatted} ${event.stablecoin.symbol}`,
        event
      );
    }
   ...
   */
  private checkTransferAnomaly(event: StablecoinEvent, amount: number): AnomalyAlert | null {
    // Transfer muito grande = HIGH
    if (amount >= this.thresholds.largeTransfer * 2) {
      return this.createAlert(
        'large_transfer',
        'high',
        `Transferência Muito Grande de ${event.stablecoin.symbol}`,
        `Detectada transferência de ${event.amountFormatted} ${event.stablecoin.symbol} de ${this.truncateAddress(event.from)} para ${this.truncateAddress(event.to)}`,
        event
      );
    }
    ...
    if (amount >= this.thresholds.largeTransfer) {
      return this.createAlert(
        'large_transfer',
        'medium',
        `Grande Transferência de ${event.stablecoin.symbol}`,
        `Detectada transferência de ${event.amountFormatted} ${event.stablecoin.symbol}`,
        event
      );
    }
   ...
   */
  private parseFormattedAmount(formatted: string): number {
