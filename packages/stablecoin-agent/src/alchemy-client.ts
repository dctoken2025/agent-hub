...
const NETWORK_MAP: Record<Network, AlchemyNetwork> = {
...
export class AlchemyClient {
  private clients: Map<Network, Alchemy> = new Map();
   ...
   */
  private getClient(network: Network): Alchemy {
   ...
   */
  async getCurrentBlock(network: Network): Promise<number> {
  ...
  async getTransferEvents(
    ...
    try {
      ...
      const events: RawTransferEvent[] = logs.map(log => {
   ...
   */
  async getTotalSupply(stablecoin: StablecoinConfig): Promise<bigint> {
    const client = this.getClient(stablecoin.network);
   ...
   */
  async getBlockTimestamp(network: Network, blockNumber: number): Promise<Date> {
    const client = this.getClient(network);
   ...
   */
  async testConnection(network: Network = 'ethereum'): Promise<{ success: boolean; blockNumber?: number; error?: string }> {
    try {
      const client = this.getClient(network);
