...
export const financialRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/stats', { preHandler: [authMiddleware] }, async (request) => {
    ...
    try {
      ...
      const byCreditorType: Record<string, { count: number; amount: number }> = {};
      (creditorResult as any[]).forEach((row: any) => {
      ...
      const upcoming = (upcomingResult as any[])[0] || { count: 0, amount: 0 };
  ...
  app.patch('/items/:id/status', { preHandler: [authMiddleware] }, async (request, reply) => {
    ...
    const { status, paidAt, notes } = request.body as { 
      status: string; 
      paidAt?: string;
      notes?: string;
    };
  ...
  app.get('/dashboard', { preHandler: [authMiddleware] }, async (request) => {
    ...
    try {
      ...
      return {
        ...
        urgentItems: urgentResult as any[],
        upcomingItems: upcomingResult as any[],
  ...
  app.post('/update-overdue', { preHandler: [authMiddleware] }, async (request) => {
    ...
    try {
      ...
      const updated = (result as any[]).length;
