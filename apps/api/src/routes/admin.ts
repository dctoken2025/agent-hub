...
export const adminRoutes: FastifyPluginAsync = async (app) => {
  ...
  // ===========================================
  app.get<{ Params: { id: string } }>('/users/:id/agents', async (request, reply) => {
    const db = getDb();
    if (!db) {
      return reply.status(500).send({ error: 'Banco de dados não disponível' });
    }
    ...
    try {
      ...
      const agentContexts = (userConfig?.agentContexts as Record<string, string | null>) || {};
      ...
      const recentActivity = await db
        .select({
          ...
          count: sql<number>`count(*)::int`,
          lastRun: sql<Date>`max(${agentLogs.createdAt})`,
          successful: sql<number>`count(*) FILTER (WHERE success = true)::int`,
          failed: sql<number>`count(*) FILTER (WHERE success = false)::int`,
        })
        .from(agentLogs)
        .where(
          and(
            eq(agentLogs.userId, id),
            gte(agentLogs.createdAt, twentyFourHoursAgo)
          )
        )
  ...
  // ===========================================
  app.post<{ Params: { id: string } }>('/users/:id/stop-agents', async (request, reply) => {
    const db = getDb();
    if (!db) {
      return reply.status(500).send({ error: 'Banco de dados não disponível' });
    }
  ...
  // ===========================================
  app.post<{ Params: { id: string } }>('/users/:id/start-agents', async (request, reply) => {
    const db = getDb();
    if (!db) {
      return reply.status(500).send({ error: 'Banco de dados não disponível' });
    }
