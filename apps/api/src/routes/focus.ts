...
export const focusRoutes: FastifyPluginAsync = async (app) => {
  ...
  // ===========================================
  app.post<{ Body: { scope?: 'today' | 'week' } }>('/refresh', { preHandler: [authMiddleware] }, async (request, reply) => {
    const userId = request.user!.id;
 ...
 */
async function getBriefing(userId: string, scope: 'today' | 'week', reply: unknown) {
  const db = getDb();
  if (!db) {
    return (reply as { status: (code: number) => { send: (data: unknown) => unknown } }).status(500).send({ error: 'Banco de dados não disponível' });
  }
  ...
  try {
    ...
    const [cached] = await db
      ...
      .where(
        and(
          eq(focusBriefings.userId, userId),
          eq(focusBriefings.scope, scope),
          gte(focusBriefings.expiresAt, now)
        )
      )
    ...
    return (reply as { status: (code: number) => { send: (data: unknown) => unknown } }).status(500).send({ 
 ...
 */
async function generateBriefing(userId: string, scope: 'today' | 'week') {
  const db = getDb();
 ...
 */
async function collectData(userId: string, scope: 'today' | 'week'): Promise<CollectedData> {
  const db = getDb();
  if (!db) throw new Error('Banco de dados não disponível');
  ...
  const emails = await db
    ...
    .where(
      and(
        ...
        eq(classifiedEmails.isArchived, false),
        or(
          eq(classifiedEmails.isRead, false),
          eq(classifiedEmails.requiresAction, true)
        ),
        or(
          eq(classifiedEmails.priority, 'urgent'),
          eq(classifiedEmails.priority, 'attention')
        )
      )
    )
  ...
  const financial = await db
    ...
    .where(
      and(
        ...
        eq(financialItems.status, 'pending'),
        lte(financialItems.dueDate, endDate)
      )
    )
  ...
  const legal = await db
    ...
    .where(
      and(
        ...
        eq(legalAnalyses.status, 'pending')
      )
    )
 ...
 */
async function analyzeWithAI(data: CollectedData, scope: 'today' | 'week') {
 ...
 */
function buildPrompt(data: CollectedData, scopeDescription: string, currentDate: string): string {
  ...
  if (data.emails.length > 0) {
    ...
    data.emails.forEach((email, i) => {
      ...
      if (email.deadline) prompt += `   Prazo mencionado: ${email.deadline}\n`;
  ...
  if (data.financialItems.length > 0) {
    ...
    data.financialItems.forEach((item, i) => {
      ...
      if (item.requiresApproval) prompt += `   ⚠️ REQUER APROVAÇÃO\n`;
      prompt += `\n`;
    });
  }
  ...
  if (data.legalItems.length > 0) {
    ...
    data.legalItems.forEach((item, i) => {
      ...
      if (item.requiredAction) prompt += `   Ação necessária: ${item.requiredAction}\n`;
      if (item.actionDeadline) prompt += `   Prazo: ${item.actionDeadline}\n`;
      if (item.isUrgent) prompt += `   ⚠️ URGENTE\n`;
      prompt += `\n`;
    });
  }
 ...
 */
function parseAIResponse(content: string, originalData: CollectedData, scope: 'today' | 'week') {
  try {
    ...
    const prioritizedItems: FocusItem[] = (parsed.items || []).map((item: Record<string, unknown>) => {
      const focusItem: FocusItem = {
        id: item.id as number,
        type: item.type as FocusItem['type'],
        title: item.title as string,
        description: item.description as string,
        urgencyScore: item.urgencyScore as number,
        urgencyLevel: item.urgencyLevel as UrgencyLevel,
        urgencyReason: item.urgencyReason as string,
        originalData: findOriginalData(item.id as number, item.type as string, originalData),
      };

      const original = focusItem.originalData;
      if (item.type === 'financial' && original.amount) {
        focusItem.amount = original.amount as number;
      }
      if (item.type === 'financial' && original.dueDate) {
        focusItem.deadline = new Date(original.dueDate as string);
      }
      if (item.type === 'task' && original.deadlineDate) {
        focusItem.deadline = new Date(original.deadlineDate as string);
      }
      if (original.stakeholderName) {
        focusItem.stakeholder = original.stakeholderName as string;
      }
    ...
    const urgentCount = prioritizedItems.filter(
...
function findOriginalData(id: number, type: string, data: CollectedData): Record<string, unknown> {
  switch (type) {
    case 'email':
      return (data.emails.find(e => e.id === id) || {}) as Record<string, unknown>;
    case 'task':
      return (data.tasks.find(t => t.id === id) || {}) as Record<string, unknown>;
    case 'financial':
...
function createFallbackBriefing(data: CollectedData, scope: 'today' | 'week') {
  ...
  data.emails
    ...
    .forEach(email => {
      items.push({
        ...
        originalData: email as unknown as Record<string, unknown>,
      });
    });
  ...
  data.tasks
    ...
    .forEach(task => {
      items.push({
        ...
        originalData: task as unknown as Record<string, unknown>,
      });
    });
  ...
  data.financialItems
    ...
    .forEach(fin => {
      items.push({
        ...
        originalData: fin as unknown as Record<string, unknown>,
      });
    });
...
function getExpirationDate(scope: 'today' | 'week'): Date {
  const now = new Date();
  if (scope === 'today') {
