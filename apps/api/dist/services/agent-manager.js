 ...
 */
import { AgentScheduler, Notifier, configureAIClient, setUsageSaveFunction } from '@agent-hub/core';
import { EmailAgent } from '@agent-hub/email-agent';
import { LegalAgent } from '@agent-hub/legal-agent';
import { FinancialAgent } from '@agent-hub/financial-agent';
import { TaskAgent } from '@agent-hub/task-agent';
import { StablecoinAgent } from '@agent-hub/stablecoin-agent';
import { CommercialAgent } from '@agent-hub/commercial-agent';
 ...
 */
async function invalidateFocusBriefings(userId) {
    const db = getDb();
    if (!db)
        return;
    try {
...
export class AgentManager {
    ...
    async initializeForUser(userId) {
        ...
        configureAIClient({
            provider: globalConfig.ai?.provider || 'anthropic',
        ...
        const agentSet = { userId, scheduler };
            ...
            try {
                ...
                if (userStablecoins.length > 0) {
                    ...
                    stablecoinAgent.onEventDetected = async (event) => {
                        try {
                            const stablecoin = userStablecoins.find((s) => s.address.toLowerCase() === event.stablecoin.address.toLowerCase());
                            if (!stablecoin)
                                return;
                    ...
                    stablecoinAgent.onAnomalyDetected = async (alert) => {
                        try {
                            const stablecoin = alert.event
                                ? userStablecoins.find((s) => s.address.toLowerCase() === alert.event.stablecoin.address.toLowerCase())
                                : null;
                    ...
                    console.log(`[AgentManager] ✅ Stablecoin Agent iniciado para usuário ${userId} (${stablecoinConfigs.length} tokens)`);
                }
                else {
                    console.log(`[AgentManager] ⚠️ Stablecoin Agent: Nenhuma stablecoin cadastrada para usuário ${userId}`);
                }
            }
            catch (error) {
    ...
    setupAgentLogging(agent, userId) {
        ...
        agent.on('completed', async (event) => {
            const db = getDb();
            if (!db)
                return;
            try {
                ...
                if (agentId.includes('email-agent')) {
                        ...
                        try {
                            await db.insert(agentLogs).values({
                                ...
                                details: {
                                    ...
                                    categories: emailData.actionItems.map((item) => item.category),
                                },
                            });
                        }
                        catch (logError) {
                        ...
                        try {
                            await db.insert(agentLogs).values({
                                ...
                                details: {
                                    ...
                                    types: emailData.commercialItems.map((item) => item.type),
                                },
                            });
                        }
                        catch (logError) {
                    ...
                    if (processedCount > 0) {
                        ...
                        if (financialData?.items && financialData.items.length > 0) {
                            try {
                                await saveFinancialItemsToDatabase(financialData.items, userId);
     ...
     */
    async startAgent(userId, agentType) {
        const agentSet = this.userAgents.get(userId);
        if (!agentSet) {
     ...
     */
    async stopAgent(userId, agentType) {
        const agentSet = this.userAgents.get(userId);
        if (!agentSet) {
     ...
     */
    async runAgentOnce(userId, agentType, input) {
     ...
     */
    async updateAgentConfig(userId, _agentType) {
     ...
     */
    async stopAll() {
        const stopPromises = Array.from(this.userAgents.keys()).map((userId) => this.stopForUser(userId));
     ...
     */
    async setAgentsActiveState(userId, active) {
        const db = getDb();
        if (!db)
            return;
        try {
     ...
     */
    async generateDailyFocusBriefings() {
        const db = getDb();
        if (!db)
            return;
        try {
