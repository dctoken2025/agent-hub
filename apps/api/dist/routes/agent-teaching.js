...
export const agentTeachingRoutes = async (app) => {
    ...
    app.post('/generate-questions', { preHandler: [authMiddleware] }, async (request, reply) => {
...
Gere exatamente 5 perguntas, cada uma com 3 opções de resposta. As perguntas devem ser diretas e objetivas em português brasileiro.`,
    ...
    app.post('/save-context', { preHandler: [authMiddleware] }, async (request, reply) => {
...
Responda APENAS com o texto do contexto, sem formatação JSON ou markdown.`,
            ...
            // Atualiza o contexto
            const currentContexts = currentConfig?.agentContexts || {
                email: null,
                legal: null,
                financial: null,
                stablecoin: null,
    ...
    app.get('/context/:agentId', { preHandler: [authMiddleware] }, async (request, reply) => {
        const userId = request.user.id;
        ...
        try {
            ...
            const [config] = await db
                ...
                .from(userConfigs)
                .where(eq(userConfigs.userId, userId));
            const contexts = config?.agentContexts || {};
    ...
    app.delete('/context/:agentId', { preHandler: [authMiddleware] }, async (request, reply) => {
        const userId = request.user.id;
        const { agentId } = request.params;
        const db = getDb();
        if (!db) {
...
//# sourceMappingURL=agent-teaching.js.map