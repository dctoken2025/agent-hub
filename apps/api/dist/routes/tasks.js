...
export const taskRoutes = async (app) => {
    ...
    app.get('/stats', { preHandler: [authMiddleware] }, async (request) => {
      ...
      `));
            return {
                ...
                byProject: projectsResult,
                byStakeholder: stakeholdersResult,
            };
        }
        catch (error) {
    ...
    app.patch('/items/:id/status', { preHandler: [authMiddleware] }, async (request, reply) => {
        const userId = request.user.id;
        const { id } = request.params;
        const { status, responseText } = request.body;
        const db = getDb();
        if (!db) {
            return reply.status(500).send({ error: 'Banco de dados não disponível' });
        }
        try {
            ...
            await db.update(actionItems)
                .set(updateData)
                .where(and(eq(actionItems.id, parseInt(id)), eq(actionItems.userId, userId)));
            return { success: true };
        }
        catch (error) {
    ...
    app.get('/dashboard', { preHandler: [authMiddleware] }, async (request) => {
      ...
      `));
            return {
                criticalItems: criticalResult,
                overdueItems: overdueResult,
                todayItems: todayResult,
    ...
    app.delete('/items/:id', { preHandler: [authMiddleware] }, async (request, reply) => {
        ...
        try {
            const result = await db
                .delete(actionItems)
                .where(and(eq(actionItems.id, parseInt(itemId)), eq(actionItems.userId, userId)))
