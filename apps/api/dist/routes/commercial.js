...
export const commercialRoutes = async (app) => {
    ...
    app.patch('/items/:id/status', { preHandler: [authMiddleware] }, async (request, reply) => {
        ...
        const { status, notes, wonValue, lostReason, assignedTo } = request.body;
        const db = getDb();
        if (!db) {
    ...
    app.get('/dashboard', { preHandler: [authMiddleware] }, async (request) => {
      ...
      `));
            ...
            return {
                ...
                highPriorityItems: highResult,
                pendingQuotes: quotesResult,
