...
export const agentRoutes = async (app) => {
    ...
    app.get('/', { preHandler: [authMiddleware] }, async (request) => {
            ...
            if (db) {
                try {
                    const countResult = await db
                        ...
                        .from(agentLogs)
                        .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `email-agent-${userId}`)));
            ...
            if (db) {
                try {
                    ...
                    const countResult = await db
                        .select({ count: sql `count(*)::int` })
                        .from(agentLogs)
                        .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `legal-agent-${userId}`)));
                    const lastRunResult = await db
                        .select({ createdAt: agentLogs.createdAt })
                        .from(agentLogs)
                        .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `legal-agent-${userId}`)))
        ...
        // Financial Agent
        const financialAgentRunning = runningAgents.find((a) => a.config.id.includes('financial'));
            ...
            if (db) {
                try {
                    ...
                    const countResult = await db
                        .select({ count: sql `count(*)::int` })
                        .from(agentLogs)
                        .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `financial-agent-${userId}`)));
                    const lastRunResult = await db
                        .select({ createdAt: agentLogs.createdAt })
                        .from(agentLogs)
                        .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `financial-agent-${userId}`)))
        ...
        // Stablecoin Agent
        const stablecoinAgentRunning = runningAgents.find((a) => a.config.id.includes('stablecoin'));
            ...
            if (db) {
                try {
                    const countResult = await db
                        .select({ count: sql `count(*)::int` })
                        .from(agentLogs)
                        .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `stablecoin-agent-${userId}`)));
                    const lastRunResult = await db
                        .select({ createdAt: agentLogs.createdAt })
                        .from(agentLogs)
                        .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `stablecoin-agent-${userId}`)))
        ...
        let taskAgentLastRun = null;
        if (db) {
            try {
                const runCountResult = await db
                    .select({ count: sql `count(*)` })
                    .from(agentLogs)
                    .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `task-agent-${userId}`)));
                const lastRunResult = await db
                    .select({ createdAt: agentLogs.createdAt })
                    .from(agentLogs)
                    .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `task-agent-${userId}`)))
        ...
        let focusAgentLastRun = null;
        if (db) {
            try {
                const runCountResult = await db
                    .select({ count: sql `count(*)` })
                    .from(agentLogs)
                    .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `focus-agent-${userId}`)));
                const lastRunResult = await db
                    .select({ createdAt: agentLogs.createdAt })
                    .from(agentLogs)
                    .where(and(eq(agentLogs.userId, userId), eq(agentLogs.agentId, `focus-agent-${userId}`)))
    ...
    app.get('/metrics', { preHandler: [authMiddleware] }, async (request) => {
        ...
        try {
            ...
            const emailsResult = await db
                .select({ total: sql `COALESCE(sum(processed_count), 0)::int` })
                .from(agentLogs)
                .where(eq(agentLogs.userId, userId));
            // Última execução
