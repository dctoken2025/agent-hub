...
export const stablecoinRoutes = async (app) => {
    // ===========================================
    ...
    app.post('/', { preHandler: [authMiddleware] }, async (request, reply) => {
        ...
        try {
            ...
            const existing = await db
                .select()
                .from(stablecoins)
                .where(and(eq(stablecoins.userId, userId), eq(stablecoins.address, cleanAddress.toLowerCase()), eq(stablecoins.network, network)));
    ...
    app.post('/anomalies/:id/acknowledge', { preHandler: [authMiddleware] }, async (request, reply) => {
        const userId = request.user.id;
        const db = getDb();
        if (!db) {
            return reply.status(500).send({ error: 'Banco de dados não disponível' });
    ...
    app.get('/:id/supply', { preHandler: [authMiddleware] }, async (request) => {
        const userId = request.user.id;
        const db = getDb();
        if (!db) {
        ...
        try {
            ...
            const result = await db
                ...
                .where(and(eq(supplySnapshots.userId, userId), eq(supplySnapshots.stablecoinId, stablecoinId), gte(supplySnapshots.timestamp, since)))
    ...
    app.get('/stats', { preHandler: [authMiddleware] }, async (request) => {
        ...
        try {
            ...
            const anomaliesResult = await db
                .select({ count: sql `count(*)::int` })
                .from(stablecoinAnomalies)
                .where(and(eq(stablecoinAnomalies.userId, userId), eq(stablecoinAnomalies.isAcknowledged, false)));
            // Última verificação
