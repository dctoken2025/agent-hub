...
async function saveLegalAnalysesToDatabase(analyses, userId) {
    const db = getDb();
    if (!db) {
    ...
    for (const analysis of analyses) {
        try {
            ...
            const existing = await db
                .select()
                .from(legalAnalyses)
                .where(and(eq(legalAnalyses.documentName, analysis.documentName), eq(legalAnalyses.userId, userId)))
                .limit(1);
            if (existing.length > 0) {
    ...
    console.log(`[EmailRoutes] Total de análises jurídicas: ${savedCount} novas, ${skippedCount} já existentes`);
}
// Função para salvar itens financeiros no banco (com userId)
async function saveFinancialItemsToDatabase(items, userId) {
    const db = getDb();
    if (!db) {
    ...
    for (const item of items) {
        try {
            ...
            const existing = await db
                .select()
                .from(financialItems)
                .where(and(eq(financialItems.emailId, item.emailId), eq(financialItems.creditor, item.creditor), eq(financialItems.amount, amount), eq(financialItems.userId, userId)))
                .limit(1);
            if (existing.length > 0) {
            ...
            await db.insert(financialItems).values({
                ...
                pixKeyType: item.pixKeyType || null,
    ...
    console.log(`[EmailRoutes] Total de itens financeiros: ${savedCount} novos, ${skippedCount} já existentes`);
}
// Função para salvar action items no banco (com userId)
async function saveActionItemsToDatabase(items, userId) {
    const db = getDb();
    if (!db) {
    ...
    console.log(`[EmailRoutes] Total de action items: ${savedCount} novos, ${skippedCount} já existentes`);
}
// Função para salvar commercial items no banco (com userId)
async function saveCommercialItemsToDatabase(items, userId) {
    const db = getDb();
    if (!db) {
    ...
    console.log(`[EmailRoutes] Total de commercial items: ${savedCount} novos, ${skippedCount} já existentes`);
}
export const emailRoutes = async (app) => {
    ...
    app.get('/needs-response', { preHandler: [authMiddleware] }, async (request) => {
        ...
        try {
            const needsResponse = await db
                .select()
                .from(classifiedEmails)
                .where(and(eq(classifiedEmails.userId, userId), eq(classifiedEmails.requiresAction, true)))
                .orderBy(desc(classifiedEmails.emailDate))
                .limit(50);
    ...
    app.post('/:emailId/mark-read', { preHandler: [authMiddleware] }, async (request, reply) => {
        const userId = request.user.id;
        ...
        try {
            ...
            if (!user?.gmailTokens) {
                ...
                await db
                    .update(classifiedEmails)
                    .set({ isRead: true })
                    .where(and(eq(classifiedEmails.emailId, emailId), eq(classifiedEmails.userId, userId)));
            ...
            await gmailClient.initializeWithTokens(user.gmailTokens);
    ...
    app.post('/mark-read', { preHandler: [authMiddleware] }, async (request, reply) => {
        const userId = request.user.id;
        ...
        try {
            ...
            const emails = await db
                ...
                .from(classifiedEmails)
                .where(sql `${classifiedEmails.userId} = ${userId} AND ${classifiedEmails.priority} = ${priority} AND ${classifiedEmails.isRead} = false`);
            if (emails.length === 0) {
            ...
            for (const email of emails) {
                try {
                    ...
                    await db
                        .update(classifiedEmails)
                        .set({ isRead: true })
                        .where(and(eq(classifiedEmails.emailId, email.emailId), eq(classifiedEmails.userId, userId)));
                    markedCount++;
                }
                catch (error) {
    ...
    app.post('/archive', { preHandler: [authMiddleware] }, async (request, reply) => {
        const userId = request.user.id;
        const { priority } = request.body || {};
        if (!priority) {
            return reply.status(400).send({ error: 'Prioridade é obrigatória' });
        ...
        try {
            ...
            const emails = await db
                .select({ emailId: classifiedEmails.emailId })
                .from(classifiedEmails)
                .where(and(eq(classifiedEmails.userId, userId), eq(classifiedEmails.priority, priority)));
            if (emails.length === 0) {
    ...
    app.post('/reply', { preHandler: [authMiddleware] }, async (request, reply) => {
        const userId = request.user.id;
        try {
